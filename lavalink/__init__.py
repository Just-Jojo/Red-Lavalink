(
    ...
    connect := coro(
        lambda channel, self_deaf=False: (
            node := get_node(channel.guild.id),
            p := (yield from node.create_player(channel, self_deaf=self_deaf)),
            p,
        )[-1]
    ),
            ...
            if not node_player
                ...
                (
                    ...
                    if data == LavalinkEvents.TRACK_END
                        ...
                        if data == LavalinkEvents.TRACK_EXCEPTION
                        else (
                            (extra := raw_data.get("thresholdMs"))
                            if data == LavalinkEvents.TRACK_STUCK
                            else (
                                (extra := raw_data.get("track"))
                                if data == LavalinkEvents.TRACK_START
                                else (
                                    (
                                        extras := {
                                    ...
                                    if data == LavalinkEvents.WEBSOCKET_CLOSED
                                    else ...
                                )
                            )
                        )
    ...
    Node := type(
        ...
        (),
        {
                                        ...
                                        if msg.type in self._closers
                                            ...
                                            (
                                                (
                                                    ...
                                                    s_try(
                                                        lambda: (
                                                            op := LavalinkIncomingOp(
                                                                data.get("op")
                                                            ),
                                                            ws_ll_log.trace(
                                                                "[NODE] | Received known op: %s",
                                                                data,
                                                            ),
                                                            asyncio.create_task(
            ...
            "create_player": coro(
                lambda self, channel, *, self_deaf=False: (
                    (
                        player := self.get_player(channel.guild.id),
                        (yield from player.move_to(channel, self_deaf=self_deaf)),
                    )
            ...
            "get_player": lambda self, guild_id: (
                ...
                else throw(PlayerNotFound("No such player for that guild."))
            ),
    ...
    get_node := lambda guild_id=None, *, ignore_ready_status=False: (
        ...
        least_used is None and throw(NodeNotFound("No Lavalink nodes found.")),
        least_used,
    )[-1],
    ...
    Player := type(
        ...
        (RESTClient, discord.voice_client.VoiceProtocol),
        {
            ...
            "connect": coro(
                lambda self, *, timeout=2.0, reconnect=False, self_mute=False, self_deaf=False: (
                    [
            ...
            "move_to": coro(
                lambda self, channel, *, self_deaf=False: (
                    ...
                    setattr(self, "channel", channel),
                    (yield from self.connect(self_deaf=self_deaf)),
                    self.current
                    and (
    ...
    Track := type(
        ...
        (),
        {
            "__init__": lambda self, data: (
                None,
                setattr(self, "track_identifier", data.get("track")),
    ...
    LoadResult := type(
        ...
        (),
        {
            "__init__": lambda self, data: (
                ...
                [
                    (
                        ...
                        and (
                            ...
                            (not _continue) and self._raw.update({k: v}),
                        ),
                    )
                    for (k, v) in _fallback.items()
                ],
                setattr(self, "load_type", LoadType(self._raw["loadType"])),
                ...
                _tracks := parse_timestamps(self._raw)
